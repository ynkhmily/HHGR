...
class HypergraphConv(MessagePassing):
    ...
    """
    def __init__(self, in_channels, out_channels, num_class=1,use_attention=False, heads=2,
                 concat=True, negative_slope=0.2, dropout=0.1, bias=True,
                 **kwargs):
        ...
        if self.use_attention:
            ...
            if heads >= 2:
                self.merge = Linear(heads * out_channels, out_channels, bias=False, weight_initializer='glorot')
            # self.att = Parameter(torch.Tensor(1, heads, 2 * out_channels))
            self.att = Parameter(torch.Tensor(num_class, heads, 2 * out_channels))
        else:
            self.heads = 1
            self.concat = True
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor,
        ...
        alpha = None
        if self.use_attention and hyperedge_attr is not None:
            # assert hyperedge_attr is not None
            # assert type_mask is not None
            ...
            if type_mask is None:
                ...
                att_type = self.att[type_mask[hyperedge_index[0]]]
            x_j = hyperedge_attr[hyperedge_index[1]]
            alpha = (torch.cat([x_i, x_j], dim=-1) * att_type).sum(dim=-1)
